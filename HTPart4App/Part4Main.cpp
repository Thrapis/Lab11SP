...
int main() {
	...
	try {
		...
		BOOL(*Update)(struct HashTable* table, const void* key, int keylength, const void* newvalue, int newvaluelength);
		(FARPROC&)Update = GetProcAddress(lib, "Update");
		...
		char* (*GetKeyAsChar)(const struct Element* element);
		(FARPROC&)GetKeyAsChar = GetProcAddress(lib, "GetKeyAsChar");
		
		int (*GetValueAsInt)(const struct Element* element);
		(FARPROC&)GetValueAsInt = GetProcAddress(lib, "GetValueAsInt");
		//----------------------------------
		...
		for (int i = 0; i < 50; i++) {
			...
			cout << "Finding of Element: key = " << key_i << endl;
			try {
				struct Element* el = Get(HT, key_i, kl + strlen(integer_string) + 1);
				if (el == nullptr)
					throw "nullptr exception";
				printCharInt(el);
				int oldvalue = GetValueAsInt(el);
				cout << "Updating to: " << endl;
				Update(HT, key_i, kl + strlen(integer_string) + 1, ToCharArray(oldvalue + 1), sizeof(int) + 1);
				el = Get(HT, key_i, kl + strlen(integer_string) + 1);
				printCharInt(el);
			}
			catch (...) {
